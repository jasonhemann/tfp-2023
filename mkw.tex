...
Similarly, the soft-cut operator \rackinline{ifte} in \emph{TRS2e}
...
denoted \rackinline{s∞} as each entry is a state that results from
...
to
\rackinline{$append} what the standard \rackinline{append-map} is
...
the result of achieving the first goal.\@ \rackinline{$append-map}
...
curried parameter \rackinline{s}. We then split \rackinline{disj} into
...
\end{minted}
  \caption{Deriving \rackinline{disj} function from macro}\label{mnt:disj-function-before-derivation}
\end{listing}
...
\end{minted}
  \caption{An η-expanded and split definition of \rackinline{disj}}\label{mnt:disj-function-split}
\end{listing}

We can replace the call to \rackinline{disj₂} in
\cref{mnt:disj-function-split} by its definition in terms
of \rackinline{append∞} and perform a trivial β-reduction. The
explicit \rackinline{s} argument suggests removing the call
to \rackinline{apply} and making \rackinline{D} recursive. The result
is the version of \rackinline{D} in
\cref{mnt:disj-substituted-through}. The definition
of \rackinline{disj} remains unchanged from
\cref{mnt:disj-function-split}. In both clauses of \rackinline{D} we
combine \rackinline{g} and \rackinline{s}, this suggests constructing
that stream in \rackinline{disj} and passing it along. Adding the
trivial base case to that \rackinline{disj} yields the definition
    ...
    (cond
      ...
      (else
       (C (λ (s)
          ...
          s))))
\end{minted}
  \caption{Replacing \rackinline{apply} in \rackinline{C} function definition}\label{mnt:C-substituted-through}
\end{listing}
  ...
  (let loop ((s∞ (g₁ s)))
    (cond
      ...
      (else (lambda ()
              (loop (s∞)))))))
  \end{minted}
  \caption{A typical implementation of \rackinline{conda}}\label{mnt:conda-implementation}
\end{listing}
...
(define (A s∞ a q-and-a* s)
  ...
  \end{minted}
  \caption{A functional \rackinline{conda} implementation}\label{mnt:conda-good-re-implementation}
\end{listing}
    ...
    (let loop ((s-inf (g s)))
      (cond
        ...
        (else (lambda ()
                (loop (s-inf))))))))
  \end{minted}
  \caption{The \rackinline{once} function}\label{mnt:condu-reimplementation}
\end{listing}
...
\Cref{mnt:call-fresh-and-call-initial-state} exhibits variants from
the.

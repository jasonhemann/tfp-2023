...
focus will be on the recurring over the list \rackinline{gs}.
...
of \rackinline{append-map∞} for Kanren-language streams) the next goal
...
appears in the right fold variant of \rackinline{disj}. Basic
...
of \rackinline{conda} requires one or more conjuncts per clause and
one or more clauses. The last line of \rackinline{conda} contains the
...
(define (A s∞ a q-and-a* s)
  ...
  \end{minted}
  \caption{A typical macro implementation of \rackinline{conda} and functional reimplementation}\label{mnt:conda-implementation}
\end{listing}
...
implementing shallow streams-based implementation in an eager host
language may have no other choice, however.
...
reimplementation, removed a degree of implementation freedom and led

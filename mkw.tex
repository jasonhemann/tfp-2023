...
\item survey, for the remaining macros in \emph{The Reasoned Schemer,
    2nd Ed}'s~\cite{friedman2018reasoned} (\emph{TRS2e}) core language
  implementation, the design space of purely functional
  implementations and weigh the trade offs and real-world concerns

\item suggest practical solutions for completely eliminating those
  remaining macros.
...
performance improvement. Approximately half of the changes are
applicable to any microKanren implementation, and the other half are
necessarily awkward yet practical strategies for those platforms
lacking macro support. The source code for our re-implementation and
...
subsume the binary \rackinline{disj₂} and \rackinline{conj₂} and
should not use \rackinline{apply}. Further, they should not build any

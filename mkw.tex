...
% Without macros, a Kanren programmer could get bothered by such
% low-level details. A small programming mistake, such as incorrectly
...
% programmer away from DSL-level thinking, and back into the host
...
\end{minted}
  \caption{Derivation of \rackinline|disj| function definition from macro.}
  \label{mnt:disj-function-before-derivation}
\end{listing}

\noindent Since \rackinline|disj| produces and consumes goals, we can
η expand the definition by a curried parameter \rackinline|s|. We then
split \rackinline|disj| into two mutually-recursive procedures, to
build the variant in \cref{mnt:disj-function-split}.
...
\noindent We can replace the call to \rackinline|disj₂| in by its
...
the call to \rackinline|apply| and make \rackinline|D| recur into
itself. The result is the version of \rackinline|D| in

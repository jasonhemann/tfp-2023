...
separately, we pass in their combination as a stream. In each
...
Kanren-language streams. The function is tail recursive; we can change
the signature in the one and only external call and the recursive
call. Adding the trivial base case to \rackinline{conj}, yields the
version shown in \cref{mnt:disj-reimplementation}.
...
\noindent We do not change \rackinline{conda}'s definition again until
the version in \cref{mnt:conda-implementation}.
         ...
         (else
          ((ifte g g1
             (λ (s)
               (A (car g*) (cdr g*) s)))
           s)))))))
= { by definition of ifte and a β reduction }
(define (A g g* s)
  (cond
    ((null? g*) (g s))
    (else
...
them. We can lift that local function \rackinline{loop} to a global
definition as long as we pass all the necessary parameters. Since the
only call to \rackinline{ifs∞te} is in \rackinline{A}, we know that

...
\rackinline{conj₂} and β-reduce. We once again eta-expand the call to
...
At this point in the left-fold implementation, these calls accumulate
in a stack-like discipline, so we can simplify and pass \rackinline{g}
and \rackinline{s} together as a stream. The equivalent right-fold
implementation, however, is written in continuation-passing style. It
seems to need to somehow construct a closure for every recursive call.
Building these superfluous closures is expensive. The same closure
stacking behavior appears in the right fold variant of
\rackinline{disj}. Basic programming horse sense suggests the more
elegant variants from \cref{mnt:disj-reimplementation}.

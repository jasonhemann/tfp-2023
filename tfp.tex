...
macro-less and side-effect free microKanren kernel, we hoped that this
separation would simultaneously aid implementers when studying the
source code, and also that the functional core would make the language
easier to port to other functional hosts. To support both of those
efforts, we also chose to program in a deliberately small and workaday
set of Scheme primitives.
...
\item suggest practical solutions for completely eliminating the
  macros in those places where the most purely functional microKanren
...
\section{The \rackinline{disj}unction and \rackinline{conj}unction
...
(define-syntax conj
  (syntax-rules ()
    ...
    ((conj g₁ g₂ g* ...)
     (conj (conj₂ g₁ g₂) g* ...))))
  \end{minted}
  \caption{microKanren \rackinline{disj₂} and \rackinline{conj₂} and their wrapper macros}\label{mnt:disj2-conj2}
\end{listing}
...
then split \rackinline{disj} into two mutually-recursive functions.
...
the call to \rackinline{apply} and making \rackinline{D}
self-recursive. The definition of \rackinline{disj} remains unchanged
from before.
...
\rackinline{disj} and passing it along. Lifting \rackinline{D} to a
global definition, and adding the trivial base case to that
\rackinline{disj} yields the definition
...
the variadic function based on the macro in {mnt:disj2-conj2}, we
first η-expand and split the definition. We next substitute for the
definitions of \rackinline{conj} and \rackinline{conj₂}.
...
external call and the recursive call. Lifting \rackinline{C} to a
global definition and adding the trivial base case to
\rackinline{conj}, yields the version shown in
...
focus is on recurring over the list \rackinline{g*}. In each recursive
call, we accumulate by mapping the next goal in the list using
\rackinline{append-map∞}'s special delaying implementation of
Kanren-language streams. Unlike \rackinline{D}, the function
\rackinline{C} does not take in the state \rackinline{s}; the help
function does not need the state for conjunction.
...
definition of \rackinline{conj₂}, and β-reduce, we get stuck. From
there, we cannot (easily) replace the \rackinline{apply} call by a
recursive call to \rackinline{C}, because we are still waiting for an
\rackinline{s}. To remove the \rackinline{apply} we can only abstract
over \rackinline{s}. So the equivalent right-fold implementation needs
to somehow construct a closure for every recursive call. Building
these superfluous closures is expensive. The same closure stacking
behavior appears in the right fold variant of \rackinline{disj}. Basic
...
variable. \rackinline{call/fresh} then applies that function with the
...
a function like \rackinline{call/initial-state}. The query is itself
